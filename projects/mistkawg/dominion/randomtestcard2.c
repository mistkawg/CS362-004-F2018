...
int main() {
    ...
    int i, testPassed = 0, testFailed = 0, player, players, seed, handSize, deckSize, discardCount, actionCount;

    printf("\nStarting randomized test for Village card.\n");
    ...
    for(i = 0; i < 1000; i++) {
        ...
        //simulating that the 0 hand position is the village card
        cardVillage(game, player, 0);

        //check if extra actions given
        if(game->numActions != (actionCount+2)) {
            printf("Test #%d failed: extra actions not applied\n", i+1);
            testFailed++;
        }

        //check discard count after
        else if(game->playedCardCount != (discardCount+1)) {
            printf("Test #%d failed: Village card not succesfully discarded\n", i+1);
            testFailed++;
        }

        //check hand size after
        else if(game->handCount[player] != (handSize)) {
            printf("Test #%d failed: not enough cards drawn to hand\n", i+1);
            testFailed++;
        }

        //check deck size after
        else if(game->deckCount[player] != (deckSize-1)) {
    ...
    printf("Village test results: %d tests passed, %d tests failed\n\n", testPassed, testFailed);
}
